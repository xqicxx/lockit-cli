...
enum Commands {
    ...
    #[command(about = "Cloud sync (no password needed)")]
    Sync {
        #[command(subcommand)]
        cmd: SyncCmd,
    },
    ...
    #[command(about = "Delete vault and reset all data")]
    Reset,
}
...
enum SyncCmd {
    ...
    Pull,
    Config,
    #[command(about = "Set or show sync encryption key")]
    Key {
        key: Option<String>,
    },
}
...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Add { json, stdin, file } => {
            commands::add::run(&paths, json, stdin, file)
        }
        Commands::List { json, query } => commands::list::run(&paths, json, query),
        Commands::Show { name_or_id, json } => {
            commands::show::run(&paths, &name_or_id, json)
        }
        Commands::Edit { name_or_id } => commands::edit::run(&paths, &name_or_id),
        Commands::Delete { name_or_id, yes } => {
            commands::delete::run(&paths, &name_or_id, yes)
        }
        Commands::Reveal { name_or_id, field } => {
            commands::reveal::run(&paths, &name_or_id, &field)
        }
        Commands::CodingPlan { cmd } => match cmd {
            CodingPlanCmd::List => commands::coding_plan::list(&paths),
            CodingPlanCmd::Refresh { provider } => {
                commands::coding_plan::refresh(&paths, provider)
            }
        },
        Commands::Sync { cmd } => match cmd {
            SyncCmd::Sync => commands::sync_cmd::sync(&paths),
            SyncCmd::Status => commands::sync_cmd::status(&paths),
            SyncCmd::Push => commands::sync_cmd::push(&paths),
            SyncCmd::Pull => commands::sync_cmd::pull(&paths),
            SyncCmd::Config => commands::sync_cmd::config(&paths),
            SyncCmd::Key { key } => commands::sync_cmd::key(&paths, key),
        },
        Commands::Env { name } => commands::env_cmd::run(&paths, &name),
        Commands::Run { name, cmd } => commands::run_cmd::run(&paths, &name, &cmd),
        Commands::Export { name, json } => {
            commands::export_cmd::run(&paths, name, json)
        }
        Commands::Import { file } => commands::import_cmd::run(&paths, &file),
        Commands::Reset => commands::reset::run(&paths),
    }
}
