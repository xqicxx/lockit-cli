...
pub fn encrypt_vault_bytes(plaintext: &[u8], password: &str, params: &CryptoParams) -> Result<Vec<u8>> {
...
pub fn open_vault_bytes(encrypted: &[u8], password: &str) -> Result<OpenedVaultBytes> {
    ...
    Ok(OpenedVaultBytes {
        plaintext,
        key: VaultMasterKey { params: envelope.params, salt, key },
    })
}

pub fn seal_opened_vault_bytes(plaintext: &[u8], master_key: &VaultMasterKey) -> Result<Vec<u8>> {
    seal_with_key(plaintext, &master_key.key, &master_key.salt, &master_key.params)
}

fn seal_with_key(plaintext: &[u8], key: &[u8; KEY_LEN], salt: &[u8], params: &CryptoParams) -> Result<Vec<u8>> {
...
fn derive_key(password: &str, salt: &[u8], params: &CryptoParams) -> Result<[u8; KEY_LEN]> {
    let params = Params::new(params.memory_kib, params.iterations, params.parallelism, Some(KEY_LEN))
