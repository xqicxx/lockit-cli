use lockit_core::credential::{Credential, RedactedCredential};
use serde::Serialize;
use tabled::Tabled;
...
pub struct JsonExport<'a> {
    pub credentials: Vec<&'a Credential>,
}

pub fn print_table(credentials: &[RedactedCredential]) {
    let rows: Vec<CredentialRow> = credentials
        .iter()
        .map(CredentialRow::from_redacted)
        .collect();
    if rows.is_empty() {
        println!("(empty)");
        return;
