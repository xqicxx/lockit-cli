...
fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::List { json, query } => {
            commands::list::run(&paths, cli.password, json, query)
        }
        Commands::Show { name_or_id, json } => {
            commands::show::run(&paths, cli.password, &name_or_id, json)
        }
        Commands::Edit { name_or_id } => {
            commands::edit::run(&paths, cli.password, &name_or_id)
        }
        Commands::Delete { name_or_id } => {
            commands::delete::run(&paths, cli.password, &name_or_id)
        }
        ...
        Commands::Run { name, cmd } => {
            commands::run_cmd::run(&paths, cli.password, &name, &cmd)
        }
