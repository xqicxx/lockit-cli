...
fn test_api_key_has_four_fields() {
    let fields = credential_fields_for(&CredentialType::ApiKey);
...
fn test_preset_values_exist_for_dropdowns() {
    let fields = credential_fields_for(&CredentialType::CodingPlan);
    let provider_field = &fields[0];
    assert!(provider_field.is_dropdown());
    assert!(provider_field.presets.contains(&"openai"));
    assert!(provider_field.presets.contains(&"anthropic"));
}
