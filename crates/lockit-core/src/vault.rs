...
use crate::crypto::{
    encrypt_vault_bytes, open_vault_bytes, seal_opened_vault_bytes, CryptoParams, VaultMasterKey,
};
...
impl VaultPaths {
    ...
    pub fn platform_default() -> Result<Self> {
        let dirs =
            ProjectDirs::from("com", "lockit", "Lockit").ok_or(VaultError::NoHomeDirectory)?;
...
pub fn init_vault(paths: &VaultPaths, password: &str) -> Result<()> {
    ...
    let encrypted =
        encrypt_vault_bytes(&payload, password, &CryptoParams::default_for_new_vault())?;
...
impl VaultSession {
    ...
    pub fn get_credential(&self, id: &str) -> Result<RedactedCredential> {
        self.payload
            .credentials
            .iter()
            .find(|c| matches_id_or_name(c, id))
            .map(Credential::redacted)
    ...
    pub fn get_credential_for_export(&self, id: &str) -> Result<&Credential> {
        self.payload
            .credentials
            .iter()
            .find(|c| matches_id_or_name(c, id))
    ...
    pub fn list_credentials_for_export(&self) -> &[Credential] {
