...
use lockit_core::sync::{compute_sync_status, sha256_checksum, SyncCrypto, SyncInputs, SyncManifest, SyncStatus};
...
fn credential_type_roundtrips_and_redacts_secret_values() {
    assert_eq!("api-key".parse::<CredentialType>().unwrap(), CredentialType::ApiKey);
    assert_eq!("GITHUB".parse::<CredentialType>().unwrap(), CredentialType::GitHub);
...
fn vault_lifecycle_returns_redacted_list_and_reveals_only_explicitly() {
    ...
    let id = session.add_credential(CredentialDraft::new(
        "GITHUB_PAT",
    ...
    assert_eq!(session.reveal_secret(&id, "value").unwrap(), "ghp_abcdefghijklmnopqrstuvwxyz");
...
fn sync_manifest_checksum_status_and_sync_crypto_are_compatible() {
    ...
    assert_eq!(compute_sync_status(SyncInputs {
