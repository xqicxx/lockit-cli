use super::{
    SyncBackend, SyncError, SyncManifest, GOOGLE_DRIVE_APPDATA_FOLDER,
    GOOGLE_DRIVE_MANIFEST_FILE, GOOGLE_DRIVE_VAULT_FILE,
};
...
impl GoogleDriveBackend {
    ...
    /// token. The caller is responsible for persisting the new token.
...
impl SyncBackend for GoogleDriveBackend {
    ...
    fn upload_vault(
        ...
        let vault_id =
            self.find_or_create_file(&client, &token, GOOGLE_DRIVE_VAULT_FILE)?;
        ...
        let manifest_id =
            self.find_or_create_file(&client, &token, GOOGLE_DRIVE_MANIFEST_FILE)?;
        self.upload_file_content(
            &client,
            &token,
            &manifest_id,
